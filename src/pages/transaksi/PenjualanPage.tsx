  ...
  return (
    <AppLayout>
      <div className="flex items-center justify-between mb-6">
        <div><h2 className="text-2xl font-bold text-foreground">Penjualan</h2><p className="text-sm text-muted-foreground mt-1">Catat transaksi penjualan produk</p></div>
        <Button className="gap-2" onClick={() => setAddOpen(true)}><Plus className="w-4 h-4" />Tambah Penjualan</Button>
      </div>

      <div className="relative max-w-xs mb-4">
        <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
        <Input placeholder="Cari penjualan..." className="pl-9" value={search} onChange={(e) => setSearch(e.target.value)} />
      </div>
      ...
      <Dialog open={addOpen} onOpenChange={setAddOpen}>
        <DialogContent className="max-w-lg">
          ...
          <div className="space-y-3">
            ...
            <Label className="text-xs text-muted-foreground">Pilih produk, jumlah, dan harga — total otomatis</Label>
            {lines.map((line, idx) => (
            ...
            <Button variant="outline" size="sm" className="gap-1" onClick={() => setLines((p) => [...p, emptyLine()])}><Plus className="w-3 h-3" />Tambah Item</Button>
            <div className="flex justify-between items-center pt-2 border-t border-border">
              <span className="text-sm font-medium text-muted-foreground">Total</span>
              <span className="text-lg font-bold text-foreground">{formatCurrency(lineTotal)}</span>
            </div>
            <Button onClick={save} className="w-full" disabled={lineTotal === 0}>Simpan Penjualan</Button>
          </div>
        </DialogContent>
      </Dialog>
